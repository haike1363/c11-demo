...
void testWaitFor() {
    //�����std::launch::deferred ���ӳ�����
    boost::future<bool> fut = boost::async(
            boost::launch::async, boost::bind<bool>(do_check_prime, 1));
    std::cout << "Checking...";
    // wait_until();
    while (fut.wait_for(boost::chrono::milliseconds(500))
...
int main() {
    std::cout << "main" << __LINE__ << " tid:" << boost::this_thread::get_id()
              << std::endl;
    auto promise = test();
    promise.set_value(1);
    //testPackagedTask();
